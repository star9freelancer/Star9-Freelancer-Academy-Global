  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <div className="size-8 rounded-full border border-border flex items-center justify-center group-hover:border-primary transition-all">
            <ArrowLeftIcon className="size-4" />
          </div>
          <span className="font-mono text-[10px] uppercase tracking-widest font-bold">Back to Courses</span>
        </button>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-8">
          <div className="h-[400px] rounded-[3rem] overflow-hidden relative border border-border group shadow-2xl">
            ...
            <div className="absolute inset-x-0 bottom-0 p-10 space-y-4">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-white/70 font-mono text-[9px] uppercase tracking-widest">{course.duration} TOTAL</span>
               </div>
               <h1 className="text-4xl md:text-5xl font-bold tracking-tighter italic text-white">{course.title}</h1>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="p-6 rounded-[2.5rem] bg-card border border-border space-y-1 group hover:border-primary transition-all">
                   <stat.icon className="size-4 text-muted-foreground mb-2 group-hover:text-primary transition-colors" />
        ...
        <div className="space-y-8 flex flex-col h-full">
                ...
                {isEnrolled ? (
                  <div className="space-y-6">
                    <div className="space-y-3">
                        ...
                        <div className="h-2 w-full bg-background rounded-full overflow-hidden border border-border p-[1px]">
                           <motion.div 
                             initial={{ width: 0 }}
           ...
           <div className="space-y-4">
               <div className="flex items-center justify-between ml-2">
                  <h3 className="text-xs font-mono uppercase tracking-[0.4em] text-muted-foreground font-bold">Lessons List</h3>
                  <span className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest">{lessons.length} Parts</span>
               </div>
               <div className="space-y-3">
                 {lessons.map((lesson, idx) => {
                   ...
                   return (
                     <motion.button
                       ...
                       disabled={!isEnrolled}
                       className={`w-full text-left p-6 rounded-[2rem] border transition-all flex items-center gap-5 group ${isEnrolled ? "bg-card border-border hover:bg-accent/50" : "bg-muted/40 border-border/50 opacity-50 cursor-not-allowed"}`}
                     >
                       <div className="size-10 rounded-2xl bg-muted border border-border flex items-center justify-center shrink-0 relative">
                          <span className="text-[10px] font-mono font-bold text-muted-foreground">{idx + 1}</span>
                          {isCompleted && (
                            <div className="absolute -top-1 -right-1 size-5 bg-emerald-500 rounded-full flex items-center justify-center border-2 border-background">
                               <ShieldCheckIcon className="size-3 text-white" />
                            </div>
                          )}
                       </div>
                       <div className="flex-1">
                          <p className="font-bold text-sm tracking-tight text-foreground">{lesson.title}</p>
                       </div>
                       {isEnrolled && (
                         <div className="size-10 rounded-full border border-border flex items-center justify-center transition-colors group-hover:bg-primary group-hover:border-primary">
                            <PlayCircleIcon className="size-5 text-muted-foreground group-hover:text-white" />
                         </div>
                       )}
                    </motion.button>
                  );
